...
#if 1             //宏条件编译选择不同的信号类型
ADCn_Ch_e   tsl1401_ch[] = {ADC0_SE14,     ADC0_SE17,    ADC1_DP1};       //PTE1  CCD所用的 ADC通道(原始信号)
#else
ADCn_Ch_e   tsl1401_ch[] = {ADC0_SE8,     ADC0_DM0,    ADC1_DM1};       //PTE2   CCD所用的 ADC通道(放大信号)
#endif
PTXn_e      tsl1401_si[] = {PTC6 ,        PTE4,       PTE10};          //CCD所用的 SI管脚
PTXn_e      tsl1401_clk[] = {PTC9 ,         PTE5,       PTE12};         //CCD所用的 SI管脚                   !!C7
PTXn_e      tsl1401_led[] = {PTA8,        PTA9 ,       PTD15};         //CCD 补光 所用的 LED_EN 管脚

//CCD 采集回来的存储空间指针数组
uint8 *tsl1401_addr[2] = {0};                                 //CCD 采集图像的地址数组
