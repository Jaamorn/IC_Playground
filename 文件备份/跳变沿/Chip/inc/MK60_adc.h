...
//外部函数接口声明
extern void     adc_init  (ADCn_Ch_e);              //AD初始化
extern uint16      ad_once   (ADCn_Ch_e, ADC_nbit);    //采集一次一路模拟量的AD值

extern uint16      ad_mid    (ADCn_Ch_e, ADC_nbit);    //中值滤波后的结果
extern uint16      ad_ave    (ADCn_Ch_e, ADC_nbit, uint8 N); //均值滤波后的结果
extern uint16      ad_flt    (ADCn_Ch_e, ADC_nbit);    //与前几次采样均值滤波   速度快于ad_ave很多，如果相邻采样时间比较短，则采样精确于 ad_once。即采样间距较短的情况下优先考虑使用这个

extern void     adc_start (ADCn_Ch_e, ADC_nbit);    //开始adc转换
