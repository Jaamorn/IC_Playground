...
void distinguish(void)
{
  ...
  while (prebin[len_hinder_1] != 0 && len_hinder_1>=64)
  {
    ...
    if (lossflag1_Left == 1 && lossflag2_Left == 1)
  {
    lossline_Left = 1;
  }
    if (lossflag1_Right == 1 && lossflag2_Right == 1)
  {
    lossline_Right = 1;
  }
...
void CarDircAdjust(void)
{
  ...
  */
  /*
    else  if(lossline_Left == 1||lossline_Right == 1)
    {
      for (k = 0;k<=2;k++)
      {
        sum+= predismid[k];
  ...
  //tsl1401_led_dis(TSL1401_MAX);             //关闭补光灯
     ...
     ///采集 线性CCD 图像
      ...
      if (shizi == 1 && shizicount != 0)
      {
       ...
       {
        CCD_BUFF[2*TSL1401_MAX+0][copynum] = prebin2[copynum];
       }
